/**
 * Convierte montos numéricos en moneda peruana a formato en letras oficial:
 * Ej: 17000.00 -> "SON: DIECISIETE MIL CON 00/100 SOLES"
 */
...
export function numeroALetrasSoles(cantidad: number): string {
  ...
  if (entero === 0) {
    return `SON: CERO CON ${strCentavos}/100 SOLES`;
  }

  const letras = Millones(entero);
  return `SON: ${letras} CON ${strCentavos}/100 SOLES`;
}
